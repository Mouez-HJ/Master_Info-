...
new(struct _cmd *c)
{
  ...
  if(mbr.nb_volumes == MAX)
    {
      printf("Plus de volumes ajoutables.\n");
      return;
    }
  printf("la taille du volume?\n");
  scanf("%d", &size);
 
  printf("0 pour base\n");
  printf("1 pour annexe\n");			      
  printf("2 pour other\n");

  printf("le type du volume\n");
  scanf("%d", &type);

  creer_partition(size,type);
...
del(struct _cmd *c){
  int volume_number;
  printf("supprimer quel volume?\n");
  scanf("%d", &volume_number);
  supprimer_partition(volume_number);  
